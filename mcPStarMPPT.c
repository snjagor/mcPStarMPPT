...
int main(int argc, char *argv[]) {
	...
	short logn=0; short tnum=0; long lint=0; long lbuf=0; //: logs...
	//--BUFFERs----------------------:
			...
			//--Number of logs to get: 			//short tnum, long lint
		 	...
		 	while (i < argc) { //printf("\t\t%d>arg[ %s ]\n", i, argv[i]); //--loop remaining log cmds--
				...
				//--days to go backwards from now:
					...
					if (lint<1 || lint>255) { printf(">unsupprted log time.\n\n"); return -1; }
    ...
    if (modbus_connect(ctx) == -1) {
        if (!sil) { fprintf(stderr, "Connection failed: %s\n", modbus_strerror(errno)); }
		else { fprintf(stdout, "-1\n");  }
        modbus_free(ctx);
        if (mrk!=5) return -1;  //-:no connection.
		//--offline actions: mrk=5==offline profile creation. 
		else action="backupprofile"; //-:new profile using builtin defs. [uncomment for prod!]
		//else if (strcmp(action,"debugc")!=0) action="backupprofile"; //-:[offline dev debug path!]
    }  
...
//---Logs:
static void *cdLabels[] = { &&cacheDParse0, &&cacheDParse1, &&cacheDParse2 }; //, &&cacheDParseDebug
//--debug logs block:
if (strcmp(action,"debugc")==0) { // && debug > 2
	...
	if (mrk) { 	nume = sizeof(eprom) / sizeof(eprom[0]); int y=0;
...
//if (action[0] && strcmp(action,"current_settings")==0){action="backupprofile"; goto print_out_profile;}//offline debug!!
			...
			//--Add to global updates[]:--------------------------------------------------------: 
				...
				if (display) { //
					...
					printf("0x%04X =  %s %s [%d],\t\"%s\"\n", profileIn[i].hexa, strReplace(".000000",".00",bufs), 
						eprom[e].unit, eprom[e].f_new, (eprom[e].string[0]?eprom[e].string:"")); 
	...
	int tmp[2] = {0x0036,0x0037}; upRegisters(ctx, ram, num, tmp, 2); 
	 short x=0; x = getIndex(0x0037,num,ram); //--:Global! int?
	now = ram[x].value.lv; x=0; //:now !!!
	/* ####################################--Header--#########################################-: */
	...
	if (real_date_logs) {  //...avoid late night? prefer afternoon?
		if (tmStruct->tm_hour > 4 && tmStruct->tm_hour < 23){ 
				latest = writeLogCache(1); }
		else {  latest = writeLogCache(0); } //create if (!logcachefile)
		if (latest && latest < (now-48)) { writeLogCache(1); } //--:update anyway
	} //* */
	...
	if (trigger_coils) { x=0;  printf("\n\nTriggering Coil Resets: \n");
		...
		for (short ii=0; ii< (int)upC; ii++) { short ci;
			...
			if (coil_updates[ii]==0x0013) { x=1;  coil_updates[ii] = 0x0000; ci = 0; } 
		...
		if (x) {	memset(&coil_updates[0], '\0', sizeof(coil_updates));  coil_updates[0] = 0x0024; 
	...
	short y=0; x = 1; //:x declared under Header
  if (! skip_RAM) {  	//-RAM---
  	//--Build Bulk Block:
	bulk[0].start = ram[0].hexa; bulk[0].i = 1;
	for (int ii=1;ii<num;ii++) { 
		if (ram[ii].hexa == (bulk[y].start+x)){ bulk[y].i++; x++; } 
		else { y++; bulk[y].start = ram[ii].hexa; bulk[y].i = 1; x=1; }  
	} // * /
	...
	/* ################################## Read the EEPROM ####################################  */
	//----Build Bulk Read:
	bulk[0].start = eprom[0].hexa; bulk[0].i = 1; y=0; x = 1;
	for (int ii=1;ii<nume;ii++) { 
		if (eprom[ii].hexa == (bulk[y].start+x)){ bulk[y].i++; x++; } 
	...
	fprintf(fp,"Median Charge Volts:	%.3f v  		SOC led:		%s\n",  
			(ram[getIndex(0x0043,num,ram)].value.fv ? (ram[getIndex(0x002A,num,ram)].value.fv*1000) / ram[getIndex(0x0043,num,ram)].value.fv:0.0), ram[getIndex(0x003B,num,ram)].value.sv); 
	...
	{ //-:logBlock: if(strcmp(action,"logs")==0) 
		struct logStruct logs [255];     //ex. logs[x].log[0].value.lv => hrs , logs[x].log[y].typ => value
		long search=0; uint start=0; //int logical; //Only start is needed!......
		x=0; char strbuf [256]=""; 
		//--Log Cache scope variables: 
		...
		if (tmStruct != NULL) { hrofday=tmStruct->tm_hour+1;  c_time -= tmStruct->tm_min*60; //0-59 ???c_time???????
			...
			//if (display && hrofday < 3) { nNow = (now - hrofday) - nH; 
			//		normT = (c_time-(hrofday*3600))-(nH*3600); x=1;	} //-:c_time targetD buffer
			//else{ //--increase*/decrease to normalize
			nNow = (24-hrofday); normT = (c_time+(nNow*3600))-(nH*3600);  nNow = (now + nNow) - nH;//def:6pm 
		} //} 
		if (debug) printf(">localtime hr: %d [%d] -> normalizing hours to %ld\n\n",tmStruct->tm_hour,hrofday,nNow);
		...
		if (logOpt==2){ 
			...
			if (logCache[0].xt && !lbuf_input) {  lbuf=0; 	
				...
				else if (cacheDate && cindex<0) {   cindex=cindex*-1; } //-:still required  
		...
		if (!logs[0].log[0].value.lv){ if (!sil) printf("\nNO LOGS!...found!\n" 
				"\n================================================================\n"
				"================================================================\n\n"); return -1; } 
		 size_t logsx = sizeof(logs)/sizeof(logs[0]);//(size_t)255
		...
		for (short a=0; a<(int)logsx; a++) { //read all logs <255 !!!!-----------------------------//--:
			if (!logs[a].log[0].hexa){ if (!sil){printf("...Last log\n");} break; } //last
			if (!logs[a].date){ logDate=0; //--no date:----------------------------:
				...
				//-LogCache: skip first search-opt log, +skip if newer than cache: 1st cache search for..:
					...
					if (!cindex || cindex==1 || (cindex>logCache[0].xt && !lbuf) || (a && logDate < logs[a-1].log[0].value.lv)) { 
						...
						else if (cindex<0) cindex=cindex*-1; 
					...
					//---:before: cindex>.xt & nextCD==0, after: cindex==0 & nextCD==0, 
						...
						if (debug>1) printf(">>>Nothing more doing for cache date. before or after logCache.\n");  //>1
					...
					} //else {}  //--same logCache period: use unaltered lbuf 		
		...
		} //--close json logs array and file:
		if (json) { writeFile ("logs", c_time, "]}\n"); printf(">Logs written to json..\n\n"); return 0; }
		//-:close
		printf("\n----------------------------------------------------------------\n\n\n\n");
		return 0;
		...
		* ------------------------------------------------------------------------------------------// */
		...
		/*/--Multi-LogCache Search: search precision, get accurate dates.. matched to external cache log
		 ...
		 if (debug>1) printf("parsing [%d] in goto block:\n",cdMarker); //" %ld",targetD
		 	//*/--Before cache Dates:-------------------------------------------------: // */
			if (cindex>logCache[0].xt) { if (debug) printf(">Target Date is before any log cache dates.\n"); 
				...
				nextCD=0; lbuf=timeSkipB; //--:lbuf - last known buffer... 
			...
			//*/--Found Match! date cache:--------------------------------------------: // */
				...
				if (debug) { printf(">>Found exact match!\n"); 
					printf(">>lbuf: %ld , timeSkipB: %ld\n\n",lbuf,timeSkipB);  }
			}
			
			//*/--Search found possibly close matches:--------------------------------: // */
			else if (cacheDate && cindex) { 	cindex=cindex*-1;  nextCD=cindex-1;   
				//--:target gt previous, lt next-----------------------------:   !round down lbuf!
				//--Find which cache ts date/buffer is better base for date... refactor lbuf:
				//notes:-------------------------------:  ..lbuf==timeSkipB most of time...
				//(previous+nextCache)=total hrs;     //if (time b/t != hrs b/t)-: 
				...
				if (cdMarker==0||cdMarker==-1) { //init-ing search:   
					...
					timeSkipA = (c_time-logCache[nextCD].udate)/3600 - (now-logCache[nextCD].hrmtr);
				  	timeSkipB = (c_time-logCache[cindex].udate)/3600 - (now-logCache[cindex].hrmtr);
					//--logCache dates nonexact!  neg values due to normalization, fixed below.
					  //--subtract logCache[cindex].nHrs: ?!?!!! 
					  //timeSkipA -= logCache[nextCD].nHrs;  timeSkipB -= logCache[cindex].nHrs;
					//--Diff b/t target date and cache dates: ¡¡targetD date is OLDEST possible!!
					...
					/* //-update lbuf to bring hrsago to correct date.:     subtract logCache[cindex].nHrs ?!?!! */
					timeSkipA = (long) (c_time-logCache[nextCD].udate)/3600 - (now-logCache[nextCD].hrmtr);
					timeSkipB = (long) (c_time-logCache[cindex].udate)/3600 - (now-logCache[cindex].hrmtr);
				...
				if (debug>1 && (logCache[nextCD].udate-logCache[cindex].udate)/3600 != (logCache[nextCD].hrmtr-logCache[cindex].hrmtr)) { long expDn = (logCache[nextCD].hrmtr-nextCache);
					printf(">TimeSkip in hrmtrs: %ld ts != %ld hrm -  [%ld]from later %s\n",((logCache[nextCD].udate-logCache[cindex].udate)/3600), (logCache[nextCD].hrmtr-logCache[cindex].hrmtr), expDn, 
							(expDn<logCache[cindex].hrmtr?"(expected date is less than prev!)\n":"(ok)")); 
				} //--:DEBUG! ---------------------------- */
				...
				if (expectedD>logCache[nextCD].hrmtr || previous>504) { //&& previous>nextCache
					lbuf = timeSkipA;  //-!!!! timeSkipB is displayed buffer!
			...
			//----Before: cindex>.xt & nextCD==0, After: cindex==0 & nextCD==0, Exact: nextCD can eq 0!----
			//--Final Debug:---------------------:
...
static short readLogs(modbus_t *ctxx, uint start,short tnum,LogObj *logs) { 
	...
	if (tnum>maxLogs) { leftovers = tnum%maxLogs;  //--:last bulk[].i      
		 if (debug>1 && leftovers) printf(">...leftovers -- %d\n",leftovers); 
		for (int uu=0; uu<tnum/maxLogs; uu++) { bulk[bi].start = start+((uu*maxLogs)*16);
			if (lp && bulk[bi].start>=bulk[0].start) { if (debug>1){printf(">...last bulk[ %d ]...\n",bi);} 
	...
	//--Done creating Bulk[]:
	if (debug>1){ printf(">...Total bulk  -  - %d\n",remaining); }  //exit(1);
	...
	for (short b=0; b<bulkT; b++) { bi=0; if(debug>1)printf(">>>Bulk [%d]\n",b); //-:sizeof(bulk)
		...
		//--SkipAhead... & fill rest of bulk[b] logs--------------------------:
			...
			while(!marker){ 
				if (start>0x8FF0) { start=0x8000; lp=1; } //-:wrap
				...
				//--skip ahead and find next non-null log:-----:
			...
			if (debug>1) printf(">!!skipping found:%d  (next start: 0x%04X , lx:%d)\n",total,start,lx);
			if (lp && start>=bulk[0].start) {break;}
			...
			if (bi>=bulk[b].i) { logx+=(px-logx); 
				if (b+1<bulkT && bulk[b+1].i){ bulk[b+1].start=start; //--:update next .start
					if (debug>1){printf(">!!skip! continue to next Bulk: start: 0x%04X\n",start);} } 
				continue; }
			...
			logx+=(px-logx);    //--- px=>next logx , bi=># of bulk[b].i , total=<tnum , -----------------
				if (debug>1) printf(">!end of skipping! start: 0x%04X , total:%d\n",start,total);
			//goto next bulk[]..
...
static const long writeLogCache(char dool) {
	...
	else if (!dool) { latestHr = readLogCache("lastLog"); e=2;  //fclose (fpp); -:Init file or return..
		if (display && latestHr > (now-24)) { printf("Log Cache: current.\n\n");  }
		else if (display) { printf("Log Cache: on.\n\n");  }
	} 
	else { //fclose(fpp); --read latest cache date, only write if gt 24 hours ago...
		latestHr = readLogCache("lastLog"); //
		if (latestHr && latestHr < (now-24)) { //--Write data to file: && latestHr != now
...
static const long readLogCache(char doo[]) {
	...
	if (fpp)  {  char str[128]=""; int tmp=0; 
		...
		if (rebuildCache) { //--backup old file_logCache:
			char ftmp[sizeof(file_logCache)+24]; snprintf(ftmp,sizeof(ftmp),"%s.%ld.bak",file_logCache,c_time);
...
static short createProfile(char doo[], int nume, RamObj *eprom0, char ctime_s[]) {
	...
	FILE *fp; char file[128]=""; char bu[64]; char buf[255]; short f=0;
	//--Date: passing ctime_s instead of recreating here...
	
	//--Open Output:-------------------------------------------:
	if (doo[0] && debug<3) { 
		//--File name: doo=="new" OR  profileBackups_ts ----: "current_settings" is global 
		if (strcmp(doo,"new")==0) {  f=1; strncpy(bu,doo,(size_t)64); } //profile=="new" 
	...
	if (f<3){ fclose(fp); printf(">Created settings backup profile [ %s ].\n", file); 
	} else { printf("\n"); }
	return 0;
}
...
static char parseProfileValue(char action[], int pi, int ep, RamObj *eprom) {
	...
	//--Normal Numbers: (secs,mins,days, ) ------------------------:
		...
		//--Check for null uint16_t: ????? more nulls??  --F16 can have but not these (probably)--
		...
		/* /--time: both checks on profileIn[pi].value.fv done already!
		} else if ( (strcmp(eprom[ep].unit,"secs")==0 || strcmp(eprom[ep].unit,"mins")==0) && (profileIn[pi].value.fv  < 0 || eprom[ep].f_new > 65535) ) { skip=1; fprintf(stderr,"> 0x%04X = %s %s !WARNINGS! new time [ %d ] is not supported!\n", 
									profileIn[pi].hexa,profileIn[pi].sv,eprom[ep].unit,(int)profileIn[pi].value.fv); //%hu  */
		//--Misc Settings:
