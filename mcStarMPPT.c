...
int main(int argc, char *argv[]) {
	...
	minV=minV>13?11.5:(minV<1?11.5:minV); 		//--min acceptable v. safety check (lithium can be low?)
...
//if (action[0] && strcmp(action,"current_settings")==0){action="backupprofile"; goto print_out_profile;}//offline debug!!
	if (strcmp(action,"profile_update")==0 || strcmp(action,"validate")==0) { //backup FIRST!!
		...
		for (int i=0;i < UPDATEABLE;i++) { 
			...
			signed char ym = parseProfileValue("up", i,e,eprom); //use above e...
			if (ym < 0) continue;  //just send eprom[ii]
		...
		/*/--Multi-LogCache Search: search precision, get accurate dates.. matched to external cache log
		 ...
		 if (debug>1) printf("parsing [%d] in goto block:\n",cdMarker); //" %ld",targetD
			...
			//*/--Found Match! date cache:--------------------------------------------: // */
				...
				if (debug) { printf(">>Found exact match (lbuf: %ld , timeSkipB: %ld)\n\n",lbuf,timeSkipB);  } //>1???????
			}
